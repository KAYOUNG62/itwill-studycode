...
-- describe(desc) 테이블이름; : 테이블 구조를 확인하기 위한 "오라클" 명령어.
-- describe 는 SQL 표준 문법의 명령어가 아님.
desc EMP;

-- 오라클의 데이터 타입:
-- (1) number: 숫자타입(정수, 실수)
--    사용예: number(2) - 두자리 정수  number(7,2) - 소수점 2자리까지 표현 가능한 실수.
-- (2) varchar, 'varchar2':가변 길이 문자열.
--    사용예: varchar2(10), varchar2(10 byte) - 최대 10byte까지 저장할 수 있는 문자열. 
--    사용예: varchar2(10 char) - 최대 10글자까지 저장할 수 있는 문자열
