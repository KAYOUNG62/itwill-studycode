...
-- ad 별명에서는 큰따옴표("")를 사용.
select deptno as "부서번호", dname as "부서 이름" 
from dept;
...
-- SQL에서 문자열은 작은 따옴표('')를 사용
select deptno || ' ' || dname as "부서 정보"
from dept;

-- dept 테이블에서 부서이름(dname)과 위치(loc)를 검색
-- 출력양식 예: ACCOUNTING 부서는 NEW YORK에 있습니다.
select dname || '부서는 ' || loc || '에 있습니다' as "부서위치"
from dept;

-- 정렬해서 출력하기:
-- select 컬럼이름, ... from 테이블이름 order by 정렬할 컬럼 [asc/desc];
-- asc: 오름차순 정렬(ascending) / desc: 내림차순 정렬(descending) 
-- 정렬의 기본은 오름차순. asc는 생략가능. 내림차순 정렬에서는 desc를 명시.

-- 부서 테이블의 모든 레코드를 부서번호 오름차순/내림차순 출력
