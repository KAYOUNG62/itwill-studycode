...
public class DbcpTest {
    ...
    public void testHikariCP() throws SQLException {
        ...
        // Connection Pool(DataSource) 생성을 위한 환경 설정 세팅.
        ...
        config.setUsername("scott"); // 접속계정
        config.setPassword("tiger"); // 접속 비밀번호
        
        config.addDataSourceProperty("cachePrepStmt", "true"); // 캐쉬기능을 이용할것인지
        config.addDataSourceProperty("prepStmtCacheSize", "250"); // 캐쉬기능 사이즈는 어떻게 할것인지
        config.addDataSourceProperty("prepStmtCacheSqlLimit", "2048"); // 캐쉬기능의 sql .limit는 어떻게 할것인지.
        
       // 위의 설정들을 갖는 Connection Pool(Data Source) 객체 생성.
        HikariDataSource ds = new HikariDataSource(config);
        Assertions.assertNotNull(ds); // Data Source는 null이 되면 안됨
        ...
        // 사용했던 Connection을 Connection Pool 에 반환
        conn.close();
