...
// 특정 클래스를 상속해야만 하거나, 상속 후엔 메서드들을 반드시 override 해야만 하는 클래스가 아님

@Slf4j
@Controller // 컴포넌트 에너테이션 중 하나. 웹 요청을 처리하는 controller 계층의 컴포넌트
// -> @Controller 에너테이션 + servlet-context.xml 설정 <context:component-scan>
...
public class ExampleController {
    ...
    // (1) 컨트롤러 메서드가 문자열을 리턴하면, 문자열이 파일의 이름. (/WEB-INF/views/문자열.jsp)
    // (2) 컨트롤러 메서드가 리턴하지 않는 경우(void), 요청 주소 이름이 뷰의 이름이됨
    ...
    public void ex2(String username, int age, Model model) {
        ...
        // -> model 속성(attribute) 이름은 JSP에서 EL로 사용할 수 있음.
        model.addAttribute("username", username);
        model.addAttribute("age", age);
    }
    
    @PostMapping("/ex3")
    public String ex3(@RequestParam(name = "username") String name, // (1)
            @RequestParam(defaultValue = "0") int age, // (2)
            Model model) {
        // 컨트롤러 메서드를 선언할 때, 파라미터 선언 앞에 @RequestParam 에너테이션을 사용.
        // (1) 메서드 파라미터와 요청 파라미터 이름이 다를때, name 속성 설정.
        // (2) 요청파라미터의 값이 없거나 비어있을 때, dafaultValue 속성 설정. 
        
        log.info("ex3(username={}, age={})", name, age);
    ...
    public String ex4(UserDto user, Model model) {
        log.info("ex4({})", user);
        // DispatcherServlet의 단계
        // 1. DispatcherServlet이 요청 파라미터들을 분석(request.getParameter),
        // 2. UserDto의 기본 생성자를 호출해서 객체를 생성
        // 3. 요청 파라미터들의 이름으로 UserDto이 setter 메서드를 찾아서 호출.
