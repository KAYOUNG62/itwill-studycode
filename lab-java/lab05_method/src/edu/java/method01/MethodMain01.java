...
// 프로그램에서 반복되는 기능을 코드 블록으로 작성한것. 
// 자바는 클래스 안에서만 함수를 작성할 수 있다. 클래스 밖에서는 함수를 작성할 수 없음.
// 자바는 메서드 안에서 메서드를 정의할 수 없다. 

//argument : 인수. 메서드를 호출하는 곳에서 메서드에게 전달하는 값.
//parameter : 매개변수, 인자. argument를 저장하기위한 메서드의 지역 변수. 
//return value : 반환값. 메서드가 기능을 모두 수행한 후에 메서드를 호출한 곳으로 반환하는 값
// 메서드는 리턴값이 있을수도 있고 없을 수도 있다.

// 메서드 정의 (선언) 방법 : 
//[수식어] 리턴타입 메서드이름([파라미터 선언,...]) { ... } //[] > 생략가능한부분 표시
// void : 함수가 리턴하는 값이 없을때 사용하는 리턴타입. 
