...
public interface JdbcSql {
    ...
    // 전체검색 - 블로그 글번호 내림차순 정렬
    String SQL_SELECT_ALL = String.format("select * from %s order by %s desc", 
            TBL_BLOGS, COL_BLOG_NO);
    
    // 블로그 글 번호로 검색
    String SQL_SELECT_BY_NO = String.format("select * from %s where %s = ?", 
            TBL_BLOGS, COL_BLOG_NO);
    
    // 블로그에 새 글 저장.
    String SQL_INSERT = String.format("insert into %s (%s, %s, %s) values (?, ?, ?)", 
...
//            TBL_BLOGS, COL_BLOG_NO);
    ...
    // format 안에서 % % 으로 감쌀 수 없고 ? 으로 처리 후 stmt.setString("%" + 값 + "%") 
