...
import static edu.java.ojdbc.OracleJdbc.*; // import static > static으로 선언된 변수/메서드 이름을 import 
import static edu.java.ojdbc.model.Blog.Entity.*;

/*
 * JDBC(Java Database Connectivity):
 * 자바 프로그램에서 데이터베이스를 연결하고 SQL 문장들을 실행하는 방법.
 * 1. Oracle JDBC 라이브러리(ojdbc8-x.x.x.x.jar)를 다운로드라고 자바 프로젝트에 추가.
 *      (1) 라이브러리 다운로드
 *      (2) 이클립스 자바 프로젝트에 lib 폴더 생성.
 *      (3) lib 폴더에 다운로드한 jar 파일을 복사
 *      (4) 복사한 jar 파일을 빌드 패스(build path)에 추가
 * 2. 오라클 데이터베이스 시스템에 접속하기 위한 정보들(URL, USER, PASSWORD, ...) 을 상수로 정의. > oraclejdbc 생성
 * 3. 오라클 JDBC 라이브러리를 사용할 수 있또록 메모리에 로드
 * 4. 오라클 데이터베이스 시스템에 접속(connection)
 * 5. Statement 객체 생성 - 데이터베이스 서버에서 실행할 sql문장 작성, 실행할 수 있는객채
 * 6. Statemnet 객체의 메서드를 사용해서 SQL 문장을 데이터베이스 서버로 전송하고, 그 결과를 받음.
 *      (1) executeQuery() - select
 *      (2) executeUpdate() - insert, update, delete
 * 7. 결과처리 - 화면출력.
 * 9. 사용했었던 모든 리소스 해제(close)
 * 
 * 
 */
...
public class JdbcMain01 {
    ...
    public static void main(String[] args) {
        ...
        // finally 구문에서 close를 해야 할 리소스 객체들
        ...
        try {
            ...
            // JDBC 에서는 SQL 문장 끝에 세미콜론(;)을 사용하지 않음.
