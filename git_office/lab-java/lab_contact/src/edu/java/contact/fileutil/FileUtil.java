...
// 도우미 클래스 - 다른 클래스에서 파일 관련(read, write) 기능을 제공하기 위한 클래스.
// 객체 생성 없이 모든 메서드들을 사용할 수 있게 하기 위해 도우미 클래스 생성
public class FileUtil {
    // 상수 정의 - 데이터 폴더 이름, 데이터 파일 이름.
    // 다른 클래스에서 사용하기위해 public, 변경이 되면 안되기 때문에 final
    public static final String DATA_DIR = "data";
    public static final String DATA_FILE = "contacts.dat";

    // private 생성자 - 다른 클래스에서 객체 생성이 안되도록 하기 위해서.
    // 대신 모든 메서드들은 public static 으로 선언.
    private FileUtil() {
    }

    /**
     * 연락처 데이터 파일을 저장하는 폴더가 없으면 폴더를 생성하고 File 객체를 리턴. 
     * 폴더가 생성되어 있으면 그 폴더에 File 객체를 리턴
     * 
    ...
    /**
     * argument로 전달된 File 객체를 사용해서, 파일에 저장된 연락처 정보를 읽고 그 결과를 List 객체로 리턴.
     * 
...
//        }
        ...
        try (FileInputStream in1 = new FileInputStream(file);
                ...
                ObjectInputStream oin1 = new ObjectInputStream(bin1);
                ){
            list = (List<Contact>) oin1.readObject();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return list;
    ...
    /**
     * argument로 전달된 테이터를 파일에 write.
     * 
     * @param data 파일에 쓸 데이터. Contact 타입을 저장하는 리스트.
     * @param file 파일 정보(파일 경로)를 가지고 있는 File 타입 객체.
     */
