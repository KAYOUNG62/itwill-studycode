...
// 클래스(class): 객체를 생성하기 위한 설계도. 데이터 타입.(비유) 붕어빵틀
// 인스턴스(instance): 메모리에 생성된 객체. (비유) 만들어져 전시되어있는 붕어빵. 

// 인스턴스 멤버(변수/필드, 메서드)
// - static 수식어가 사용되지 않은 멤버.
// - 객체를 생성한 후에, 참조 변수를 이용해서 사용. (new)
...
// - Java Runtime 이 사용하는 메모리 공간 중 Method(메서드)영역에 생성됨.
// - 프로그램 로딩 시점(main 메서드 시작 전)에 메모리에 생겨남.
// - static 멤버들은 static 멤버들만 접근(사용) 가능. 
// - 지역 변수 앞에 static을 사용할 수 없다

// final 수식어 : 클래스, 멤버(필드/변수, 생성자, 메서드), 지역변수에 사용됨
// final 지역변수 : 값이 한번 저장되면 변경할 수 없는 변수.
// fianl 멤버변수: 객체가 생성될 때 값이 초기화되면 변경할 수 없는 멤버 변수.

public class ModifierMain07 {

    public static void main(String[] args) {
        // static 멤버(변수, 메서드) 는 클래스 이름으로 참조해서 사용.
        // main 메서드는 자바가상머신이 인스턴스 호출 없이 바로 사용할 수 있게 하기위해 void 앞에 static 을 붙힌다.
        ...
        // static 변수를 참조 변수를 이용해서 사용할 수는 있지만, 권장하지 않는다.
        // static 변수는 클래스 이름으로 참조하는 것을 권장.
        // 참조변수(test) , StaticTest.staticVariable 이런식으로 사용하는걸 권장.
