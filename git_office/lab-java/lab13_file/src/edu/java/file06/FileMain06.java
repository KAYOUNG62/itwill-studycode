...
/*
 * Java객체(object) 타입을 파일에 쓰거나 파일에서 읽는 방법:
 * 
 * 프로그램 ==> ObjectOutputStream ==> FileOutputStream ==> 파일
 *  ObjectOutputStream: 자바 객체를 파일에 write 할 수 있는 형태로 변환(serialize, 직렬화)
 *  
 * 프로그램 <== ObjectInputStream <== FileInputStream <== 파일
 *  ObjectInputStream: 파일에서 읽은 데이터를 자바 객체로 변환(de-serialize, 역직렬화)
 *  
 * ObjectInputStream, ObjectOutputStream에서 사용하는 자바클래스는 반드시 Serializable 인터페이스 구현.
 */
