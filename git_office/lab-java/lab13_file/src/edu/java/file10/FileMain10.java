...
public class FileMain10 {
    ...
    public static void main(String[] args) {
        // 파일과 폴더(디렉토리) 생성, 삭제, 관리

        // System.getProperty(key); : Java Runtime 이 실행중인 컴퓨터의 여러가지 시스템 정보(속성)들을 리턴.
        ...
        // (예) D:\study\lab-java\lab13-file\src\edu\java\file10\FileMain10.java
        ...
        for (File f : files) {
           ...
           String info = null;
           if (f.isFile()) { //f가 파일인 경우
               info = fileName + "\t" + f.length() + "byte";
        ...
        // 폴더(디렉토리)생성 , 삭제
        File newFolder = new File("data" , "temp"); // 상위폴더, 하위폴더(파일)
        if (newFolder.exists()) { // 이미 폴더가 만들어져있는 경우.
