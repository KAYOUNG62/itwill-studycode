...
// 인터페이스가 가질 수 있는 멤버들 - Java8 버전 이후의 변경. ([] 생략가능표시)

// 1. [public static final] 필드(멤버변수)
// 2. [public abstract] 메서드: body가 없는 추상 메서드.

// 3. [public] default 메서드: body({...})가 구현된 메서드. Java8 버전부터.
//  -> 인터페이스를 구현하는 (하위)클래스의 객체를 생성한 후에 참조 변수를 이용해서 사용하는 메서드
// 4. [public] static 메서드: body가 구현된 static 메서드. Java8 버전부터.
//  -> 객체 생성 없이 인터페이스 이름으로 참조해서 호출(사용)하는 메서드.
// 5. private static 메서드: body가 구현된 static 메서드. Java9 버전부터.
//  -> 인터페이스의 [public] static 메서드, [public] default 메서드 안에서만 사용하기 위한 목적의 메서드.
// 6. private 메서드: body가 구현된 메서드. Java9 버전부터. 
...
public class InterfaceMain02 {
    ...
    public static void main(String[] args) {
        ...
        // 인터페이스의 [public abstract] 메서드 사용 - 구현된 메서드가 있어야함. (new TestImpl)
        test.test();
