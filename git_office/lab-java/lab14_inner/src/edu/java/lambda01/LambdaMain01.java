...
//        Calculable multiplier = (x , y) -> {return x * y;}; // (double x, double y) double 생략해도됨
         ...
         * 문법 함수형 인터페이스(functional interface)만 람다 표현식으로 작성할 수 있음. 함수형 인터페이스: 추상 메서드를 오직
         * 한 개만 갖는 인터페이스. (param, ...) ->{...} 람다 표현식의 파라미터 선언에서는 타입을 생략해도 됨. 람다 표현식의
         * body{...} 에서 실행 문장이 하나만 있는 경우 {}생략해도됨 람다 표현식에서 body가 return 문장만 있는 경우에는 {}와
         * return까지 생략, 리턴값만 작성하면됨 (parm, ...) -> 리턴값 람파 표현식에서 파라미터 개수가 1개인 경우 () 생략 가능.
         */
